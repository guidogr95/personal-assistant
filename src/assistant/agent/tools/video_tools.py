...
@tool(category="🎬 Video")
async def get_video_transcript(ctx: RunContext[AgentDeps], url: str) -> str:
...
@tool(category="🎬 Video")
async def get_transcription_queue_status(ctx: RunContext[AgentDeps]) -> str:
    ...
    if recent and isinstance(recent, list):
        ...
        for job in recent:
            ...
            if job_status == "completed":
                lines.append(f"  ✅ {job.get('url', '')} → `{job.get('note_filename', 'unknown')}`")
            else:
